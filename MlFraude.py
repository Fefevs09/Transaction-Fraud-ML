...
df_test = df.drop(['ssn', 'cc_num', 'first', 'last', 'street', 'zip', 'lat', 'long', 'city_pop', 'acct_num', 'trans_num', 'merchant', 'merch_lat', 'merch_long'], axis=1)
df_test.info()
cleaning_data_datetime(df_test)
get_user_age(df_test)

one_hot = encoder.transform(df_test[categorical_data])
encoded_df = pd.DataFrame(one_hot.toarray(), columns=encoder.get_feature_names_out(categorical_data))
df_encoded = pd.concat([df_test.drop(columns=categorical_data), encoded_df], axis=1)
